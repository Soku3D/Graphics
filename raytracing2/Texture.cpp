...
Texture::Texture(int width, int height, int channels, const std::vector<glm::vec3>& pixels)
	:texturePath("Textures\\"),
	resultPath("Results\\"),
	width(width),
	height(height),
	channels(channels)
{
	...
	{ 
		...
		mPixels[idx * channels+1] = static_cast<stbi_uc>(pixels[idx].y / 255.0);
		mPixels[idx * channels+2] = static_cast<stbi_uc>(pixels[idx].z / 255.0);
	}
}
void Texture::RenderImage(const std::string filename)
{
	std::string currPath = resultPath + filename + ".png";
	stbi_write_png(currPath.c_str(), width, height, channels, mPixels.data(), width * channels);
